function buildChatPrompt(products, conditions) {
  return [
    ...
    '',
    `이 사람의 조건: 가족 수 ${conditions.familySize}인 / 자녀 ${conditions.hasChildren ? '있음' : '없음'} / 보관 공간 ${conditions.storageSpace} / 월 사용량 ${conditions.monthlyUsage}`,
    '',
    '추천 전에 아래 단계를 순서대로 따른다:',
    '',
    '[1단계] 이 사람의 실제 구매 상황을 먼저 파악한다.',
    `- 보관 공간이 ${conditions.storageSpace}이고 월 사용량이 ${conditions.monthlyUsage}인 사람에게 대용량 구매는 실제로 이득인가, 부담인가?`,
    `- 자녀가 ${conditions.hasChildren ? '있으므로' : '없으므로'} 무항생제·유기농 등 안전성 프리미엄의 가격 차이가 이 사람에게 정당한가?`,
    '- 위 질문에 답한 뒤, 이 사람의 구매 우선순위 3가지를 명확히 정한다.',
    '',
    '[2단계] 1단계에서 정한 우선순위로 각 상품을 평가한다.',
    '- 상품 설명에 "N인 가구 적합"이라고 적혀 있어도, 이 사람의 우선순위에 맞지 않으면 추천 대상에서 제외한다.',
    '- 우선순위에 어긋나는 상품은 이유를 들어 제외하고, 우선순위에 맞는 상품만 후보로 남긴다.',
    '',
    '[3단계] 후보 상품 중 이 사람에게 가장 잘 맞는 상품을 추천한다.',
    '- 추천 이유는 반드시 1단계에서 정한 이 사람의 우선순위와 연결해서 설명한다.',
    '- 상품 설명을 그대로 반복하지 않는다.',
    '',
    ...
    JSON.stringify(
      {
        ...
        recommendedProducts: ['최대 2개. 우선순위에 가장 잘 맞는 상품만 포함.',
          {
...
function buildInsightPrompt(products) {
  return [
    ...
    '- childSafetyNote: 자녀가 먹거나 피부에 닿는 상품이면, 무항생제·유기농·저온살균·무향·저자극 성분·무방부제 등 실제 안전성 속성과 관련 리뷰 내용을 한 줄로. 크기·편의성·맛은 제외. 해당 없으면 빈 문자열.',
