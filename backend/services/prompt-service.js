function buildChatPrompt(products, conditions) {
  return [
    ...
    '',
    '추천할 때 아래 기준을 사용자 조건에 맞춰 반드시 적용한다:',
    '',
    `현재 사용자 조건 요약:`,
    `- 가족 수: ${conditions.familySize}인`,
    `- 자녀 여부: ${conditions.hasChildren ? '있음 → 성분·자극·안전성 리뷰를 우선 검토한다' : '없음 → 자녀 안전성은 추천 기준에서 제외한다. 유기농·무항생제 등 프리미엄 안전성 제품을 굳이 우선하지 않는다'}`,
    `- 보관 공간: ${conditions.storageSpace === 'small' ? 'small → 대용량 상품은 추천하지 않는다. 소포장·단품을 우선한다' : conditions.storageSpace === 'large' ? 'large → 대용량 구매가 유리하면 적극 추천한다' : 'medium → 용량 선택에 유연하게 판단한다'}`,
    `- 월 사용량: ${conditions.monthlyUsage === 'low' ? 'low → 소량·단품 위주로 추천한다. 대용량은 소비 전에 유통기한이 지날 수 있다' : conditions.monthlyUsage === 'high' ? 'high → 대용량·묶음 구매가 합리적인지 검토한다' : 'medium → 상황에 맞게 판단한다'}`,
    '',
    '1. 신선도·유통기한: 식품이면 리뷰에서 언급된 수령 상태, 신선도, 유통기한 우려를 확인한다.',
    '2. 단품/대용량 실속: unitPrice와 위 조건을 함께 고려해 어떤 선택이 합리적인지 판단한다.',
    '3. 위 조건에 맞지 않는 상품은 추천하지 않는다.',
    '',
